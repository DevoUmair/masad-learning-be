...
// };
import crypto from "crypto";
// REMOVE THIS LINE: import axiosInstance from "../utils/axios.js";
...
export const generateSecureBunnyUrl = async (req, res) => {
  try {
    ...
    // Return the URL immediately. DO NOT use axios to fetch video data.
    return res.status(200).json({
      url: secureUrl,
    });

  } catch (error) {
    // This will only catch local errors now, not BunnyCDN 401s
    console.error("Error generating secure Bunny URL:", error.message);
    return res.status(500).json({ error: "Internal server error" });
  }
};